 ...
 */
let SortByTo = (courses, to) => {
    let selectedCourses  = null;

    let coursesTo        = courses.filter ( ({prices}) => prices[0] === null ); // курсы до определенной цены [null, 400]
    let coursesFrom      = courses.filter ( ({prices}) => prices[1] === null ); // курсы от определенной цены [200, null]
    let coursesFromAndTo = courses.filter ( ({prices}) => prices[0] !== null && prices[1] !== null ); // курс от и до [100, 200]
 ...
 * @param {number} to - число, до которого осуществляется поиск
 */
let SortByFromAndTo = (courses, from, to) => {
    let selectedCourses  = null;
 ...
 * 
 * @param {*} requiredRange - Варианты цен (фильтры), которые ищет пользователь
...
let SortByPrice = (requiredRange, courses) => {
    ...
    switch(RESULT) {
        ...
        case "2":
            alert("from");
            SortByFrom(courses, from);
            break;
        case "3":
            alert("from to");
            SortByFromAndTo(courses, from, to);
            break;
        default:            
            break;
    }
